 ...
 *
 * REQUIRES_NEW perche' qui siamo dopo il commit del cambio di prezzo, su un
 * thread diverso: non c'e' nessuna transazione da riusare.
 */
@Slf4j
@Service
...
public class AvvisoNotificationService {
    ...
    /**
     * Prende in carico gli avvisi che il ribasso ha attraversato e restituisce
     * quelli da spedire davvero.
     *
    ...
    public List<NotificaAvviso> prendiInCarico(PrezzoRibassatoEvent e) {
        ...
        for (Avviso a : candidati) {
            // I valori si leggono adesso: segnaInviato svuota il contesto di
            ...
            var soglia = a.getSoglia();

            if (avvisoRepository.segnaInviato(id) != 1) {
            ...
            presi.add(new NotificaAvviso(id, email, nome, marca, modello, prezzo, soglia, token));
        }
        return presi;
    }
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void rimettiInAttesa(UUID avvisoId) {
        avvisoRepository.rimettiInAttesa(avvisoId);
        log.warn("Avviso {}: rimesso in attesa dopo un invio fallito", avvisoId);
    }
}
